...
export default class Book {
  ...
  constructor() {
    this.currPageNumber = <number>workspace.getConfiguration().get('minibook.currPageNumber');
    this.currChapterNumber = <number>workspace.getConfiguration().get('minibook.currChapterNumber');
  ...
  initBookContent() {
    ...
    // 使用正则表达式匹配章节内容
    const contentRegex = /(?:.*\s)?第.+章\s*(\S+)([\s\S]*?)(?=(?:.*\s)?第.+章\s*\S+|$)/g;
    let match;
    const chapterList = [];
  ...
  updatePage() {
    workspace.getConfiguration().update('minibook.currChapterNumber', this.currChapterNumber, true);
    workspace.getConfiguration().update('minibook.currPageNumber', this.currPageNumber, true);
  }
  ...
  getTextByPage(): string {
    ...
    this.page = this.bookContent.chapterList[this.currChapterNumber - 1].pageSize;
  ...
  getPreviousPage(): string {
    ...
    if (this.currPageNumber <= 1) {
      ...
      this.currPageNumber = this.bookContent.chapterList[this.currChapterNumber - 1].pageSize;
      this.initChatper();
    } else {
